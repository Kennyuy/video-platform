<template>
  <div>
    <div class="flex justify-between items-center mb-6">
      <h2 class="text-2xl font-bold text-gray-900 dark:text-white">编辑音频</h2>
    ...
    <div v-else-if="audio" class="card p-6 max-w-2xl">
      <form @submit.prevent="handleUpdate" class="space-y-6">
        ...
        <div>
          <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">音频封面</label>
          <div class="flex items-start gap-4">
            ...
            <div class="w-40 h-24 bg-gray-100 dark:bg-slate-800 rounded-lg overflow-hidden">
              <img v-if="coverPreview || audio.cover_url" :src="coverPreview || audio.cover_url" alt="封面预览" class="w-full h-full object-cover" />
    ...
    <div v-else class="text-center py-12">
      <p class="text-gray-600 dark:text-gray-400">音频不存在或无权访问</p>
    </div>
  </div>
</template>
...
const loadAudio = async () => {
  ...
  try {
    ...
    audio.value = await userAudioApi.getAudioDetail(id)
    form.value.title = audio.value.title
    form.value.description = audio.value.description || ''
    form.value.visibility = audio.value.visibility
  } catch (error) {
...
const handleUpdate = async () => {
  ...
  try {
    ...
    const response = await fetch(`/api/user/audios/${audio.value!.id}`, {
      method: 'PUT',
      headers: {
...
const handleDelete = async () => {
  if (!confirm('确定要删除这个音频吗？此操作不可恢复。')) return

  try {
    await userAudioApi.deleteAudio(audio.value!.id)
    alert('删除成功')
    router.push('/studio/audios')
  } catch (error) {
