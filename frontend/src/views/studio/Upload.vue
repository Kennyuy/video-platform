    ...
    <el-card>
      <template #header>
        <div class="card-header">
          <span>上传视频</span>
          <el-button @click="$router.push('/studio/videos')">
            返回稿件管理
          </el-button>
        </div>
      </template>
...
import { userVideoApi, categoryApi } from '@/api'
...
const handleVideoChange = (uploadFile: UploadFile) => {
  if (uploadFile.raw) {
    ...
    if (!allowedTypes.includes(uploadFile.raw.type)) {
      ElMessage.warning('建议使用 mp4 格式以获得最佳兼容性')
    }
...
const handleSubmit = async () => {
  ...
  try {
    if (uploadMode.value === 'file') {
      // 文件上传
      await userVideoApi.uploadVideo({
        title: form.title,
      ...
      // URL上传
      await userVideoApi.addVideoByURL({
        title: form.title,
